...
// Input : w, b, x
// Output: L, dL/dw1, dL/dw2 ... dL/dwN, dL/db, dL/dX1, dL/dX2 ... dL/dXN
neuron(N, activation, learningRate) = cross : update_params : autodiff_compute : grad_route(N)
    with {
        ...
        autodiff_compute = routing : (par(i, N, v.var(i+1),v.var(i+1+N) : d.diff(*)) : d.sumall(N)),v.var(2*N+1) : d.diff(+)
            : acti
            with {
                v = vars((par(i, 2*N+1, _)));
                d = env(v);
        ...
        // in: <L, dL/dx1, dL/dx2... dL/dxN, dL/dw1, dL/dw2... dL/dwN, dL/db>
        // out: <L, dL/dw1, dL/dw2... dL/dwN, dL/db, dL/dx1, dL/dx2... dL/dxN>
...
// In: X, Y (1 signal)
fc(N, n, activationFn, learningRate) = route(N*(n+1)+n, N*(2*n+1), 
...
// In: number of neurons (N), number of inputs (n)
// This function is only for fc layers that ARE NOT the last layer.
chainApply(N, n, x) = par(i, (2*n+1)*x, _), (route(N+2*n+1, N+2*n+1, (1,1), par(i, N-1, (i+2, 2*n+1+i+2)), par(i, 2*n+1, (i+N+1, i+2)))
                    : (route(2*n+2, 4*n+2, par(i, 2*n+1, (1, 2*i+1), (i+2, 2*i+2))): par(i, 2*n+1, *)), par(i, N-1, _)), par(i, (2*n+1)*(N-1), _);

gradAveraging(N, n) = route(N*n, N*n, par(i, n, par(j, N, (2*j+i+1, i*N+j+1)))) : par(i, n, sum(j, N, _)) : par(i, n, _ : /(N));